...
void CM2B_MOEAD::Parameter_setting(string algorithm, int ProblemID, int ProblemSubID) {
...
void CM2B_MOEAD::cout_population(int swit) {
	switch (swit) {
	case 1:
...
void CM2B_MOEAD::cout_property(int swit) {
	switch (swit) {
	case 1:
		for (int i = 0; i <num_vec; ++i) {
...
//weighted sum
double CM2B_MOEAD::weighted_sum(individual ind) {
	double value = 0;
...
double CM2B_MOEAD::tchebycheff_notabsolute(individual ind) {
	double max = 0;
	double g = 0;
...
//Tchebycheff
double CM2B_MOEAD::tchebycheff(individual ind) {
	double max = 0;
	double g = 0;
...
//PBI
double CM2B_MOEAD::PBI(individual ind) {
...
//IPBI
double CM2B_MOEAD::IPBI(individual ind) {
...
//AOF
double CM2B_MOEAD::AOF(individual ind) {
	int k = ind.weight_no;
...
double CM2B_MOEAD::scalarizing_function(individual ind) {
	if (scalar == "WS") {
...
void CM2B_MOEAD::total_constraints(individual& ind) {
	ind.total_c = 0;
...
void CM2B_MOEAD::initialize_reference_nadir_point() {
...
void CM2B_MOEAD::initialize_population() {
...
void CM2B_MOEAD::file_objectives(int k, int island) {
...
//evolution (mu, mu) 
void CM2B_MOEAD::evolution_mu(int curent_gen) {
...
//run
void CM2B_MOEAD::run_algorithm(int reps) {
	rep = reps;
	srand(reps);
	init_genrand(reps);
	//generate files
...
void CM2B_MOEAD::file_initialization(int  k, int gen) {
	int count = 0;
	int tmp_gen = 0;
...
void CM2B_MOEAD::file_allsolutions(int k, int gen, string scalar, vector<population> pop, vector<double> ras) {
	{
...
//目的関数値のプロット(現個体)
void CM2B_MOEAD::file_curents(int k, int gen, string scalar, int island) {
...
int CM2B_MOEAD::calc_cossim(individual offspring1) {
	double a, b, ab;
	double min_cos = 0;
	int min_vec = 0;
...
int CM2B_MOEAD::tournament_PRandCV(int tournamentSize, population pop) {
...
int CM2B_MOEAD::tournament_totalC(int tournamentSize, population pop) {
	vector<int> candidates;
	int candidate = 0;
	double min_totalC = 1000000;
	int min_rank = 10000000;
...
int CM2B_MOEAD::tournament(int tournamentSize, population pop) {
	vector<int> candidates;
	int candidate = 0;
...
int CM2B_MOEAD::tournament_fitness(int tournamentSize, population pop) {
	vector<int> candidates;
	int candidate = 0;
...
int CM2B_MOEAD::tournament_inverse(int tournamentSize, population pop) {
	vector<int> candidates;
	int candidate = 0;
...
individual CM2B_MOEAD::SBX(int vec1, int vec2, int n1, int n2) {
...
individual CM2B_MOEAD::DE(int vec1, int vec2, int vec3, int vec4, int n1, int n2, int n3, int n4) {
	individual offspring1, offspring2;
	double mu = 20.0;//distribution index
	double SBX_u = 0;
	double beta1, beta2;
...
void CM2B_MOEAD::PM(int vec, int pp) {
...
void CM2B_MOEAD::update_NSGAII_const(vector<individual> update_population, int vec) {
	vector<int> frontID;
...
void CM2B_MOEAD::update_NSGAII(vector<individual> update_population, int vec) {
	vector<int> frontID;
	vector<vector<int>> flag2;
	int flag_dominated = 0;
	frontID.resize(update_population.size());
...
int CM2B_MOEAD::get_minID(population pop) {
	double min_totalC = 1000000000;
	double min_fitness = 1000000000;
	int minID = 0;
	for (int i = 0; i < pop_m; ++i) {
...
void CM2B_MOEAD::parentselection(int vec,int& i1, int& i2, int& n1, int& n2) {
	double delta = 0.5;
