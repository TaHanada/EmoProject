...
void CM2B_MOEAD3::Parameter_setting(string algorithm, int ProblemID, int ProblemSubID) {
	...
	case 3:
		...
		break;
	default:
		//only competition problem or water problem
...
void CM2B_MOEAD3::cout_population(int swit) {
	switch (swit) {
	case 1:
...
void CM2B_MOEAD3::cout_property(int swit) {
	switch (swit) {
	case 1:
		for (int i = 0; i <num_vec; ++i) {
...
//weighted sum
double CM2B_MOEAD3::weighted_sum(individual ind) {
	double value = 0;
...
double CM2B_MOEAD3::tchebycheff_notabsolute(individual ind) {
	double max = 0;
	double g = 0;
...
//Tchebycheff
double CM2B_MOEAD3::tchebycheff(individual ind) {
	double max = 0;
	double g = 0;
...
//PBI
double CM2B_MOEAD3::PBI(individual ind) {
...
//IPBI
double CM2B_MOEAD3::IPBI(individual ind) {
...
//AOF
double CM2B_MOEAD3::AOF(individual ind) {
	int k = ind.weight_no;
...
double CM2B_MOEAD3::scalarizing_function(individual ind) {
	if (scalar == "WS") {
...
//制約違反量の総和だと各制約によるスケールの問題がありそう
//calcurate total constraints
void CM2B_MOEAD3::total_constraints(individual& ind) {
	ind.total_c = 0;
...
void CM2B_MOEAD3::initialize_reference_nadir_point() {
...
void CM2B_MOEAD3::initialize_population() {
	vector<individual> update_population;

	// 初期個体群生成と評価，reference&nadirの更新
	for (int i = 0; i < num_vec; ++i) {
	...
	// 各重みベクトルごとにスカラー関数値を算出してサブ問題に投げる
	for (int i = 0; i < num_vec; ++i) {
...
void CM2B_MOEAD3::file_objectives(int k, int island) {
...
//evolution (mu, mu) 
void CM2B_MOEAD3::evolution_mu(int curent_gen) {
...
//run
void CM2B_MOEAD3::run_algorithm(int reps) {
	rep = reps;
	srand(reps);
	init_genrand(reps);
	//generate files
	...
	cout << "initialize refandnad" << endl;
...
void CM2B_MOEAD3::file_initialization(int  k, int gen) {
	int count = 0;
	int tmp_gen = 0;
...
void CM2B_MOEAD3::file_allsolutions(int k, int gen, string scalar, vector<population> pop, vector<double> ras) {
	{
...
//目的関数値のプロット(現個体)
void CM2B_MOEAD3::file_curents(int k, int gen, string scalar, int island) {
...
int CM2B_MOEAD3::calc_cossim(individual offspring1) {
	double a, b, ab;
	double min_cos = 0;
	int min_vec = 0;
...
int CM2B_MOEAD3::tournament_PRandCV(int tournamentSize, population pop) {
...
int CM2B_MOEAD3::tournament_totalC(int tournamentSize, population pop) {
	vector<int> candidates;
	int candidate = 0;
	double min_totalC = 1000000;
	int min_rank = 10000000;
...
int CM2B_MOEAD3::tournament(int tournamentSize, population pop) {
	vector<int> candidates;
	int candidate = 0;
...
int CM2B_MOEAD3::tournament_fitness(int tournamentSize, population pop) {
	vector<int> candidates;
	int candidate = 0;
...
int CM2B_MOEAD3::tournament_inverse(int tournamentSize, population pop) {
	vector<int> candidates;
	int candidate = 0;
...
individual CM2B_MOEAD3::SBX(int vec1, int vec2, int n1, int n2) {
...
individual CM2B_MOEAD3::DE(int vec1, int vec2, int vec3, int vec4, int n1, int n2, int n3, int n4) {
	individual offspring1, offspring2;
	double mu = 20.0;//distribution index
	double SBX_u = 0;
	double beta1, beta2;
...
void CM2B_MOEAD3::PM(int vec, int pp) {
...
void CM2B_MOEAD3::update_NSGAII_const(vector<individual> update_population, int vec) {
	vector<int> frontID;
...
void CM2B_MOEAD3::update_NSGAII(vector<individual> update_population, int vec) {
	vector<int> frontID;
	vector<vector<int>> flag2;
	int flag_dominated = 0;
	frontID.resize(update_population.size());
...
//				fout << curent[vec].ind[pp].fitness << "\t" << curent[vec].ind[pp].total_c << endl;
	...
	// アーカイブにランク0の解を入れる
	if (count_a + flag2[0].size() < pop_a) {
		for (int i = 0; i < flag2[0].size(); ++i) {
				archive[0].ind[count_a] = update_population[flag2[0][i]];
				archive[0].ind[count_a].rank = 0;
				count_a++;
			}
	}
	else {
		...
		vector<double> cd;
		cd.resize(count_a);
		vector<vector<double>> cd2;
		cd2.resize(count_a, vector<double>(2));
		vector<double> ni;
		double r = 0;
		// ni.resize(num_obj*count_a);
		vector<vector<double>> dist_archive;
		dist_archive.resize(count_a, vector<double>(count_a));
		// 全ての解間のユークリッド距離を求める
		int count_d = 0;
		for (int i = 0; i < count_a; ++i) {
			for (int j = 0; j < count_a; ++j) {
				...
				if (i == j) {
					...
					for (int k = 0; k < num_obj; ++k) {
						dist_u = dist_u + pow(archive[0].ind[i].cost[k] - archive[0].ind[j].cost[k], 2);
					}
					dist_u = sqrt(dist_u);
				}
				//cout << dist_u << endl;
				
				dist_archive[i][j] = dist_u;
				/*
				if (dist_u == 0) {
					count_d++;
				}
		...
		//cout <<"count"<< count_d << endl;//2553もあるからそりゃあ上位5個だと0ばっかにもなるか
		// ニッチ半径を求める
		//cout << "nisize" << ni.size() << endl;
		for (int i = 0; i < count_a; ++i) {
			sort(dist_archive[i].begin(), dist_archive[i].end());
			for (int j = 1; j < num_obj + 1; j++) {
				ni.push_back(dist_archive[i][j]);
				// cout << dist_archive[i][j] << endl;
			}
		}
		//cout << "nisize2:" << ni.size() << endl;
		if (ni.size() % 2 == 0) {
			r = (ni[ni.size() / 2 - 1] + ni[ni.size() / 2]) / 2;
		}
		else {
			r = ni[(ni.size() - 1) / 2];
		}
		...
		for (int i = 0; i < count_a; ++i) {
			for (int j = 1; j < num_obj + 1; j++) {
				if (dist_archive[i][j] <= r) {
					R = R * dist_archive[i][j] / r;
				}
				else {
					R = R * 1;
				}
			...
			cd2[i][0] = i;
			cd2[i][1] = 1 - R;
			// cout << 1 - R << endl;//  -nan(ind)
		}
		// 混雑度大きい奴消して非劣解入れる
		sort(cd.begin(), cd.end(), std::greater<double>());
		int count_n = 0;
		if (pop_a - count_a != 0) {
			int k = pop_a - count_a;
			for (int i = 0; i < k; ++i) {
			...
			while (k + count_n < flag2[0].size()) {
				for (int i = 0; i < pop_a; ++i) {
					if (cd2[i][1] == cd[count_n]) {
						archive[0].ind[i] = update_population[flag2[0][k + count_n]];
						archive[0].ind[i].rank = 0;
						count_n++;
					}
				}
			}
			...
			while (count_n < flag2[0].size()) {
				for (int i = 0; i < count_a; ++i) {
					if (count_n < flag2[0].size()) {
						if (cd2[i][1] == cd[count_n]) {
							archive[0].ind[i] = update_population[flag2[0][count_n]];
							archive[0].ind[i].rank = 0;
							count_n++;
...
void CM2B_MOEAD3::update_vector() {
	vector<double> dist_archive;
	//dist_archive.resize(num_vec*pop_m);
	int minID = 0;
	int minID2 = 0;
	vector<vector<double>> id;
	id.resize(count_a, vector<double>(2));
	// 候補解の選択
	// アーカイブ内から個体群全体で距離の最も近い解までの距離の中央値(0になりそう)
	for (int i = 0; i < count_a; ++i) {
		double min_dist_w = 100000000;
		for (int j = 0; j < num_vec; ++j) {
			for (int l = 0; l < pop_m; ++l) {
				double dist_w = 0;
				for (int k = 0; k < num_obj; ++k) {
					dist_w = dist_w + pow(archive[0].ind[i].cost[k] - curent[j].ind[l].cost[k], 2);
				}
				dist_w = sqrt(dist_w);
				if (min_dist_w >= dist_w) {
					min_dist_w = dist_w;
					minID = j;
					minID2 = l;
				}
		...
		// cout << min_dist_w << endl;// 全部0
		dist_archive.push_back(min_dist_w);
		id[i][0] = minID;
		id[i][1] = minID2;
	}
	// ニッチ半径
	double r_w = 0;
	if (dist_archive.size() % 2 == 0) {
		r_w = (dist_archive[dist_archive.size() / 2 - 1] + dist_archive[dist_archive.size() / 2]) / 2;
	}
	else {
		r_w = dist_archive[(dist_archive.size() - 1) / 2];
	}
	if (r_w == 0) {
		int count_r = 0;
		for (int i = 0; i < dist_archive.size(); ++i) {
			r_w = r_w + dist_archive[i];
			count_r++;
		}
		r_w = r_w / count_r;
	}
	cout << r_w << endl;
	// 候補解判定ーニッチ半径より大きい方が優れているとしよう
	if (r_w != 0) {
		for (int i = 0; i < count_a; ++i) {
			if (dist_archive[i] > r_w) {
...
int CM2B_MOEAD3::get_minID(population pop) {
	double min_totalC = 1000000000;
	double min_fitness = 1000000000;
	int minID = 0;
	for (int i = 0; i < pop_m; ++i) {
...
void CM2B_MOEAD3::parentselection(int vec,int& i1, int& i2, int& n1, int& n2) {
	double delta = 0.5;
