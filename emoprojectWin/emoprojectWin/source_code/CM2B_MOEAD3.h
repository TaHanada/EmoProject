...
private:
	...
	int pop_a;//archive size
	int count_a;//count archive
	...
	string scalar;
	int rep;
	vector<double> reference_point;//参照点
	vector<double> nadir_point;//最悪点

	vector<population> curent;
	vector<population> offspring;
	vector<population> archive;
...
public:
	...
	//constractor
	CM2B_MOEAD3(int tmp_rep, int ProblemID, int tmp_ProblemSubID) {
		Parameter_setting("CM2B_MOEAD3", ProblemID, tmp_ProblemSubID);
		rep = tmp_rep;
