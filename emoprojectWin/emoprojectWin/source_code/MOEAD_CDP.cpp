...
void MOEAD_CDP::Parameter_setting(string algorithm, int ProblemID,int ProblemSubID) {
	...
	default:
		...
		if (ProblemID == 6) {
			num_vec = 330;
			H1 = 7, H2 = 0; //このとき，分割数は6．
		}
		else {
			num_vec = 210;
			H1 = 6, H2 = 0;//分割数 12(M=3) 6(M=5) 3,2(M=8) 3,2(M=10)
		}
		break;
	}
...
void MOEAD_CDP::cout_population(int swit) {
	switch (swit) {
	case 1:
...
void MOEAD_CDP::cout_property(int swit) {
	switch (swit) {
	case 1:
...
//weighted sum
double MOEAD_CDP::weighted_sum(individual ind) {
	double value = 0;
...
double MOEAD_CDP::tchebycheff_notabsolute(individual ind) {
	double max = 0;
	double g = 0;
...
//Tchebycheff
double MOEAD_CDP::tchebycheff(individual ind) {
	double max = 0;
	double g = 0;
...
//PBI
double MOEAD_CDP::PBI(individual ind) {
...
//IPBI
double MOEAD_CDP::IPBI(individual ind) {
...
//AOF
double MOEAD_CDP::AOF(individual ind) {
	int k = ind.weight_no;
...
double MOEAD_CDP::scalarizing_function(individual ind) {
	if (scalar == "WS") {
...
void MOEAD_CDP::total_constraints(individual& ind) {
	ind.total_c = 0;
...
void MOEAD_CDP::initialize_reference_nadir_point() {
...
void MOEAD_CDP::initialize_population() {
...
void MOEAD_CDP::file_objectives(int k, int island) {
	string name = "data_" + to_string(k) + ".dat";
	ofstream fout("./MOEAD_CDP/" + to_string(ProblemSubID) + "/" + scalar + "/island" + to_string(island + 1) + "/" + name);

	vector<double> maxcost(num_obj, 0);
	vector<double> mincost(num_obj, 10000000);
	if (island == 0) {
		for (int i = 0; i < num_vec; ++i) {
...
//evolution (mu, 1) 
void MOEAD_CDP::evolution_one(int gen) {
...
//run
void MOEAD_CDP::run_algorithm(int reps) {
	rep = reps;
	srand(reps);
	init_genrand(reps);
	//generate files
...
void MOEAD_CDP::file_initialization(int  k, int gen) {
	int count = 0;
	int tmp_gen = 0;
	...
	//offspirngs
	{
		...
		ofstream fout("./MOEAD_CDP/" + to_string(ProblemSubID) + "/" + scalar + "/offsprings/" + name1);
		fout << "#gen	Feasible	";
		for (int i = 0; i < num_obj; ++i) {
	...
	for (int island = 0; island < 2; ++island) {
		string name1 = to_string(k) + "_" + to_string(0) + "gen.dat";
		ofstream fout("./MOEAD_CDP/" + to_string(ProblemSubID) + "/" + scalar + "/island" + to_string(island + 1) + "/curents/" + name1);
		fout << "#gen	Feasible	";
		for (int i = 0; i < num_obj; ++i) {
			fout << "f" << to_string(i + 1) << "	";
		}
...
void MOEAD_CDP::file_allsolutions(int k, int gen, string scalar, population pop) {
	{
		string name1 = to_string(k) + "_" + to_string(gen) + "gen.dat";
		ofstream fout("./MOEAD_CDP/" + to_string(ProblemSubID) + "/" + scalar + "/offsprings/" + name1, ios::out | ios::app);
...
int MOEAD_CDP::calc_cossim(individual offspring1) {
	double a, b, ab;
	double min_cos = 0;
	int min_vec = 0;
...
individual MOEAD_CDP::SBX(individual ind1, individual ind2) {
...
individual MOEAD_CDP::PM(individual ind) {
...
void MOEAD_CDP::curent_fout(int k, int j, int island) {
	string name1 = to_string(k) + "_" + to_string(j) + "gen.dat";
	ofstream fout("./MOEAD_CDP/" + to_string(ProblemSubID) + "/" + scalar + "/island" + to_string(island + 1) + "/curents/" + name1, ios::out | ios::app);
