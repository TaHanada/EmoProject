...
void MOEAD_IEpsilon::Parameter_setting(string algorithm, int ProblemID,int ProblemSubID) {
	...
	//Tc_ : T_c以降の世代はCDPと同じ挙動になる．
	//tau_ : epsilon の値を決定する式のパラメータ．
	//theta_ : 初期個体の上位何番目の個体の制約違反量で epsilon の初期値を決定するか．
	//alpha_ : rule2 から rule 3 へ移行するパラメータ．ある世代の実行可能解の割合．alpha_ より実行可能解の割合が大きければ rule 3 へ．
...
void MOEAD_IEpsilon::file_objectives(int k, int island) {
	...
	ofstream fout("./MOEAD_IEpsilon/" + to_string(ProblemSubID) + "/" + scalar + "/island" + to_string(island + 1) + "/" + name);
...
void MOEAD_IEpsilon::file_initialization(int  k, int gen) {
	...
	//offspirngs
	{
		...
		ofstream fout("./MOEAD_IEpsilon/" + to_string(ProblemSubID) + "/" + scalar + "/offsprings/" + name1);
		fout << "#gen	Feasible	";
		for (int i = 0; i < num_obj; ++i) {
	...
	for (int island = 0; island < 2; ++island) {
		string name1 = to_string(k) + "_" + to_string(0) + "gen.dat";
		ofstream fout("./MOEAD_IEpsilon/" + to_string(ProblemSubID) + "/" + scalar + "/island" + to_string(island + 1) + "/curents/" + name1);
		fout << "#gen	Feasible	";
		for (int i = 0; i < num_obj; ++i) {
			fout << "f" << to_string(i + 1) << "	";
		}
...
void MOEAD_IEpsilon::file_allsolutions(int k, int gen, string scalar, population pop) {
	{
		...
		ofstream fout("./MOEAD_IEpsilon/" + to_string(ProblemSubID) + "/" + scalar + "/offsprings/" + name1, ios::out | ios::app);
...
void MOEAD_IEpsilon::curent_fout(int k, int j, int island) {
	...
	ofstream fout("./MOEAD_IEpsilon/" + to_string(ProblemSubID) + "/" + scalar + "/island" + to_string(island + 1) + "/curents/" + name1, ios::out | ios::app);
