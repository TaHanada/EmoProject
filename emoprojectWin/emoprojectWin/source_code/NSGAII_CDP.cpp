...
void NSGAII_CDP::Parameter_setting(string algorithm, int ProblemID, int ProblemSubID) {
...
void NSGAII_CDP::cout_population(int swit) {
	switch (swit) {
	case 1:
...
void NSGAII_CDP::cout_property(int swit) {
	switch (swit) {
	case 1:
...
void NSGAII_CDP::total_constraints(individual& ind) {
	ind.total_c = 0;
...
void NSGAII_CDP::initialize_population() {
...
void NSGAII_CDP::file_objectives(int k, int island) {
	string name = "data_" + to_string(k) + ".dat";
	ofstream fout("./NSGAII_CDP/" + to_string(ProblemSubID) + "/" + scalar + "/island" + to_string(island + 1) + "/" + name);
	/*fout << "#gen	Feasible	";
	...
	vector<double> mincost(num_obj, 100000000);
	if (island == 0) {
		for (int i = 0; i < pop_m; ++i) {
...
//evolution (mu, 1) 
void NSGAII_CDP::evolution(int gen) {
...
//run
void NSGAII_CDP::run_algorithm(int reps) {
	rep = reps;
	srand(reps);
	init_genrand(reps);
	//generate files
...
void NSGAII_CDP::file_initialization(int  k, int gen) {
	int count = 0;
	int tmp_gen = 0;
	...
	//offspirngs
	{
		...
		ofstream fout("./NSGAII_CDP/" + to_string(ProblemSubID) + "/" + scalar + "/offsprings/" + name1);
		fout << "#gen	Feasible	";
		for (int i = 0; i < num_obj; ++i) {
			fout << "f" << to_string(i + 1) << "	";
		}
	...
	for (int island = 0; island < 2; ++island) {
		string name1 = to_string(k) + "_" + to_string(0) + "gen.dat";
		ofstream fout("./NSGAII_CDP/" + to_string(ProblemSubID) + "/" + scalar + "/island" + to_string(island + 1) + "/curents/" + name1);
		fout << "#gen	Feasible	";
		for (int i = 0; i < num_obj; ++i) {
			fout << "f" << to_string(i + 1) << "	";
		}
		...
		for (int i = 1; i <= tmp_gen; ++i) {
			//string name1 = to_string(k) + "_.dat";
			string name1 = to_string(k) + "_" + to_string(i) + "gen.dat";
			ofstream fout("./NSGAII_CDP/" + to_string(ProblemSubID) + "/" + scalar + "/island" + to_string(island + 1) + "/curents/" + name1);
			fout << "#gen	Feasible	";
			for (int i = 0; i < num_obj; ++i) {
...
void NSGAII_CDP::file_allsolutions(int k, int gen, population pop) {
	{
		//string name1 = to_string(k) + "_.dat";
		string name1 = to_string(k) + "_" + to_string(gen) + "gen.dat";
		ofstream fout("./NSGAII_CDP/" + to_string(ProblemSubID) + "/" + scalar + "/offsprings/" + name1, ios::out | ios::app);
...
individual NSGAII_CDP::SBX(individual ind1, individual ind2) {
...
individual NSGAII_CDP::PM(individual ind) {
...
void NSGAII_CDP::evaluation(population pop) {
...
int NSGAII_CDP::tournament(int tournamentSize, population pop) {
...
int NSGAII_CDP::check_dominance(individual a, individual b) {
...
population NSGAII_CDP::assign_CrowdingDistance(vector<int> IDs, population pop) {
...
void NSGAII_CDP::curent_fout(int k, int j, int island) {
	string name1 = to_string(k) + "_" + to_string(j) + "gen.dat";
	ofstream fout("./NSGAII_CDP/" + to_string(ProblemSubID) + "/" + scalar + "/island" + to_string(island + 1) + "/curents/" + name1, ios::out | ios::app);
